...
//			} else {
//				$this->_batchLog[] = "Unsubscribe Batch {$i|escape:'html':'UTF-8'}: {$batchResult['success_count']} Succeeded";
//				$this->_batchLog[] = "Unsubscribe Batch {$i|escape:'html':'UTF-8'}: {$batchResult['error_count']} Failed";
//				if ($batchResult['error_count']) {
//					$this->_batchErrors["Unsubscribe Batch {$i|escape:'html':'UTF-8'}"] = $batchResult['errors'];
//				}
//			}
//			unset($unsubscribe[$i]);
//		}
	...
	protected function processLocalUsers($listId)
	{
		...
		while($users = $this->getUsers($listId, $batch++, $this->_batchSize))
		{
			...
			{
				$this->_batchLog[] = "Subscribe Batch {$batch|escape:'html':'UTF-8'}: {$batchResult['success_count']} Succeeded";
				$this->_batchLog[] = "Subscribe Batch {$batch|escape:'html':'UTF-8'}: {$batchResult['error_count']} Failed";
				if ($batchResult['error_count'])
					$this->_batchErrors["Subscribe Batch {$batch|escape:'html':'UTF-8'}"] = $batchResult['errors'];
			}
		}
	}
}