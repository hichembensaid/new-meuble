...
class QuickOrderAjaxModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess(): void
    {
        ...
        if (!$_SERVER || strtoupper($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
            $this->ajaxDie(false, 'Méthode non autorisée.', 405);
        }

        // Vérification CSRF
        if (!$this->validateToken()) {
            $this->ajaxDie(false, 'Token invalide.', 403);
        }
        ...
        match ($action) {
            ...
            default       => $this->ajaxDie(false, 'Action inconnue.', 400),
        };
    }
    ...
    private function handleSubmitOrder(): void
    {
        ...
        if (!empty($errors)) {
            $this->ajaxDie(false, implode('<br>', $errors), 422);
        }
        ...
        try {
            ...
            $this->ajaxDie(true, $this->module->trans(
            ...
            $this->ajaxDie(false, $this->module->trans(
     ...
     *
     * @param array<string, mixed> $extra Données supplémentaires à inclure dans la réponse
     */
    private function ajaxDie(
        bool   $success,
        string $message,
        int    $httpCode = 200,
        array  $extra    = []
