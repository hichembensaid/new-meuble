...
class QuickOrderAjaxModuleFrontController extends ModuleFrontController
{
    ...
    private function handleSubmitOrder(): void
    {
        ...
        // ---- 2. Validation -----------------------------------------------
        $validator = new QuickOrderValidator($this->module);
        ...
        try {
            ...
            $this->sendJson(true, sprintf(
                $this->module->l('Votre commande #%s a bien été enregistrée ! Nous vous contacterons prochainement.', 'ajax'),
                $order->reference
            ...
            $this->sendJson(false, '[DEBUG] ' . $e->getMessage() . ' — ' . basename($e->getFile()) . ':' . $e->getLine(), 500);
        }
    }
