...
class QuickOrder extends Module
{
    ...
    {
        // En PS8 $params['product'] peut être un objet Product ou un tableau selon le thème
        $product = $params['product'] ?? null;

        // Récupération robuste de l'ID produit
        $productId = 0;
        $productName = '';
        ...
        // Fallback : lire l'ID depuis le controller si le hook ne le fournit pas
        if (!$productId && isset($this->context->controller->id_product)) {
            $productId = (int) $this->context->controller->id_product;
        }

        // Dernier fallback : lire depuis $_GET
        if (!$productId) {
            $productId = (int) Tools::getValue('id_product');
        }

        if (!$productId) {
            PrestaShopLogger::addLog('[QuickOrder] hookDisplayProductActions: id_product introuvable', 2);
            return '';
        }
    ...
    /**
     * Injecte le CSS + JS directement dans le footer (contourne les problèmes
     * de registerJavascript selon le thème actif).
     */
