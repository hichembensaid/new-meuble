...
$(document).ready(function(){   
   ...
   if (length_cat > 10){
    ...
    // Initialisation du menu mobile au chargement - Version renforcée
    $(window).on('load', function(){
        // Forcer la fermeture complète au chargement
        setTimeout(function(){
            $('#mobile-categories-block').removeClass('active').hide();
            $('#mobile-categories-overlay').remove();
    ...
    $(document).ready(function(){
        ...
        // Forcer l'affichage des catégories dans le menu mobile (même si display:none inline)
        $('#mobile-categories-block .category-top-menu-pos').css('display', 'block');
        $('#mobile-categories-block .category-top-menu-list').css('display', 'block');
        $('#mobile-categories-overlay').remove();
        $('#menu-icon').removeClass('opened').addClass('closed');
        $('body').css('overflow', '');
    });
    ...
    $('#menu-icon').on('click',function(){
        if ($(window).width() <= 767){
            // Afficher les catégories au lieu du menu en mobile
            ...
            if($(this).hasClass('closed'))
            {
                ...
                // Créer l'overlay s'il n'existe pas et l'insérer AVANT le menu
                if (overlay.length === 0) {
                    wrapper.before('<div id="mobile-categories-overlay"></div>');
                ...
                // Forcer l'affichage des catégories même si display:none inline
                wrapper.find('.category-top-menu-pos').css('display', 'block');
                wrapper.find('.category-top-menu-list').css('display', 'block');
                
                // Empêcher le scroll du body
                $('body').css('overflow', 'hidden');
                
                // Cacher le mega menu si présent
                $('.mm_menus_ul').hide();
            }
            else
            {
    ...
    // Fermer le menu mobile des catégories quand on clique sur le bouton de retour
    ...
    // Gérer les sous-menus dépliables dans le menu mobile
    $(document).on('click', '#mobile-categories-block .navbar-toggler', function(e){
        e.preventDefault();
        e.stopPropagation();
        ...
        if ($submenu.hasClass('show')) {
            ...
            $this.closest('li').siblings().find('.collapse.show').removeClass('show').slideUp(300);
            $this.closest('li').siblings().find('.navbar-toggler').attr('aria-expanded', 'false');
